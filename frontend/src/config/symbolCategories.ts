...
export function loadSymbolCategories(): StoredSymbolCategories {
  try {
      ...
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        return Object.fromEntries(
          Object.entries(parsed).filter(
            (entry): entry is [string, EditableSymbolCategory] =>
    ...
    // preserve the legacy star list as Watchlist while sharing one
...
export function defaultSymbolCategory(candidate: TradingSymbol): EditableSymbolCategory {
  // share the exact market-kind fallback between both symbol menus so
