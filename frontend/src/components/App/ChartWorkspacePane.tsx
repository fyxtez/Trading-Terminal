...
import { saveViewportToStorage } from "../../hooks/marketDataPersistence";
import { type SymbolApi, isTradingSymbol } from "../../hooks/useSymbol";
...
import { WorkspaceRuntimeContext } from "./workspaceRuntimeContext";
...
function ChartWorkspacePane({
  ...
  useEffect(() => {
    refs.chartRef.current?.applyOptions({ timeScale: { lockVisibleTimeRangeOnResize: split } });
  ...
  const toggleWorkspaceSplit = () => {
    if (!split) {
      // Resize the existing pane without changing its visible time range, then
      // give the new pane the same viewport rather than a different default zoom.
      const chart = refs.chartRef.current;
      chart?.applyOptions({ timeScale: { lockVisibleTimeRangeOnResize: true } });
      const x = chart?.timeScale().getVisibleLogicalRange();
      const y = refs.candleRef.current?.priceScale().getVisibleRange();
      if (x && y && refs.chartReadyRef.current) {
        const viewport = {
          xFromOffset: x.from - refs.lastLogicalIndexRef.current,
          xToOffset: x.to - refs.lastLogicalIndexRef.current,
          yFrom: y.from,
          yTo: y.to,
        };
        saveViewportToStorage(currentSymbol, marketData.interval, viewport);
  ...
  const backendConnection = runtime.backend;
  const {
    showDrawings,
    setShowDrawings,
    showAsiaSession,
   ...
   */
  const locallyRecordedMarketFillsRef = runtime.localMarketFills;
  ...
  useLayoutEffect(() => {
    positionsStore.set(paneId, {
      ...
      protection: { symbol: currentSymbol, fullTakeProfitPrice, stopLossPrice },
    });
  });
  ...
  useMobileBackDismissal(hasDismissibleMobileLayer && active, () => {
  ...
  return (
    ...
    >
      ...
      <ChartTabs
        paneLabel={split ? (paneId === "left" ? "LEFT" : "RIGHT") : undefined}
        onMoveTab={split ? onMoveTab : undefined}
        moveLabel={moveLabel}
        active={active}
        tabs={chartTabs.tabs}
        activeSymbol={currentSymbol}
...
export default ChartWorkspacePane;
